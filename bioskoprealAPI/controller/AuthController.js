...
module.exports={
    Authlogin:(req,res)=>{
        const {id}=req.params //params tuh buat apa
        const {username,password}=req.query //query mksdnya apa
        if(username&&password){
            ...
            db.query(sql,(err,results)=>{
                ...
                // console.log(token)
                return res.status(200).send({result:results,token}) //bedanya hasil result sm results
            })
        }else if(id){
            // ini buat keeplogin
